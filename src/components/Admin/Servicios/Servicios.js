...
export default function Servicios(props) {
   ...
   const addServicioModal = () => {
      ...
      setModalTitle("Crear Nueva Instalación");
      setModalContent(
         <AddEditServicio
   ...
   const deleteServicio = (servicio) => {
      ...
      confirm({
         ...
         content: `¿Estas seguro que quieres eliminar esta instalación: ${servicio.title}?`,
         okText: "Eliminar",
         okType: "danger",
         cancelText: "Cancelar",
         onOk() {
   ...
   const editServicioModal = (servicio) => {
      setIsVisibleModal(true);
      setModalTitle(`Editar Instalación: ${servicio.title}`);
      setModalContent(
         <AddEditServicio
            setIsVisibleModal={setIsVisibleModal}
            setReloadServicios={setReloadServicios}
   ...
   return (
      <div className="servicio-list">
         <div className="servicio-list__header">
            <div>
               <h4>Instalaciones</h4>
               Para ordenar: seleccione una tarjeta y arrastre hacia la posición deseada dentro de
   ...
   return (
      <List.Item
         actions={[
            ...
            //         />,
            <Button type="primary" title="Editar" onClick={() => editServicioModal(item)}>
               <EditOutlined />
            </Button>,
            <Button type="danger" title="Eliminar" onClick={() => deleteServicio(item)}>
               <DeleteOutlined />
            </Button>,
         ]}
      >
         <List.Item.Meta title={item.title} description={"Sección: " + item.section} />
      </List.Item>
   );
}
